...
| **ML Service** (AI服务) | `services/meridian-ml-service` | Python, FastAPI, Sentence-Transformers, UMAP, HDBSCAN | Docker 容器 / VPS (Systemd) |
...
> **新增依赖说明 (2026-02-14)**:
> 本服务新增了 `umap-learn` (降维) 和 `hdbscan` (聚类) 依赖。
> - **Docker 部署**: 推荐方式。镜像构建过程会自动处理依赖编译。
> - **源码部署**: 若在本地或裸机运行，必须安装 C++ 编译工具链（如 Ubuntu 的 `build-essential` 或 Windows 的 Visual Studio Build Tools），否则 `hdbscan` 安装会失败。
