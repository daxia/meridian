...
3.  **Logger 接口限制**: 项目使用的 `Logger` 库在 `error` 方法中对元数据对象（metadata）有严格的类型定义，不允许随意传入 `error` 对象作为属性，或者 `Error` 对象本身不包含我们试图访问的某些非标准属性。
...
// Before
    ...
    logger.error('...', { text, error: e.message }); // 错误：e 可能是 unknown
}
...
// After
    ...
    // 1. 截断过长的文本防止日志爆炸
    // 2. 使用 error_message 明确字段含义，避免与 Logger 内部 error 对象冲突
    logger.error('...', { 
