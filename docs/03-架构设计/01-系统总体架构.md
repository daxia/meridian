...
- **技术栈**: Python 3.11, FastAPI, Sentence-Transformers, Uvicorn, UMAP, HDBSCAN。
- **职责**:
  - 提供内部 API (如 `/embeddings`, `/cluster`)。
  - **向量化**: 运行轻量级 ML 模型 (如 `all-MiniLM-L6-v2`) 生成 384 维文本向量。
  - **聚类分析**: 使用 UMAP 降维 + HDBSCAN 算法对文章向量进行聚类，发现热点话题。
  - 部署在支持 GPU/CPU 的容器环境中 (本地 Docker 或 Fly.io)。

### 3.4 数据存储层 (Packages/Database)
- **PostgreSQL**:
  - **核心表**: `$publishers`, `$data_sources`, `$ingested_items`, `$reports`。
...
4.  DB 进行向量相似度搜索 (Cosine Distance)。
5.  返回相关文章。

### 4.3 智能情报汇总流 (Generate Brief)
1.  **触发**: 系统定时任务 (Cron `0 6,18 * * *`)，每日早晚两次。
2.  **数据准备**: Backend 拉取过去 24 小时内已处理且包含向量的文章。
3.  **聚类分析**:
    - Backend 发送所有文章向量至 ML Service (`/cluster`)。
    - ML Service 使用 UMAP + HDBSCAN 识别话题簇。
    - 返回 Cluster Labels。
4.  **摘要生成**:
    - Backend 按簇分组文章。
    - 对每个簇调用 LLM (Gemini) 生成“话题综述”。
5.  **报告生成**:
    - 汇总所有话题综述，生成 Markdown 格式简报。
    - 存入数据库 `$reports` 表。
