...
describe('parseRssFeed', () => {
  ...
  it('handles cn.nytimes.com feed', async () => {
    ...
    expect(result.value.items[0].title).toBe('前高管揭Facebook内幕：配合北京开发审查工具');
    expect(result.value.items[0].link).toBe('https://cn.nytimes.com/culture/20250318/careless-people-sarah-wynn-williams/');
    expect(result.value.items[0].pubDate).toStrictEqual(new Date('Tue, 18 Mar 2025 04:59:35 +0800'));
  });
  ...
  it('handles ft.com feed', async () => {
    ...
    expect(result.value.items[0].title).toBe('‘If Trump defies a Supreme Court order, will it matter to markets?’');
    expect(result.value.items[0].link).toBe('https://www.ft.com/content/2e579290-fc0c-4b88-8703-f0bae45266d9');
    expect(result.value.items[0].pubDate).toStrictEqual(new Date('Tue, 18 Mar 2025 23:34:47 GMT'));
  });
  ...
  it('handles theverge.com feed', async () => {
    ...
    expect(result.value.items[0].title).toBe('The Boeing Starliner astronauts returned to Earth today');
    expect(result.value.items[0].link).toBe(
      'https://www.theverge.com/news/628311/nasa-crew-10-mission-starliner-astronauts-return-spacex'
    );
    expect(result.value.items[0].pubDate).toStrictEqual(new Date('2025-03-18T18:04:44-04:00'));
  });
});
