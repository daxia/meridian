...
| 目录               | 文件名格式                                                           | 说明                                                                                                                                                                                                                                                                                                | 示例                                                                                                      |
| ------------------ | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| 01-我的需求        | **我的需求.md**（原则上唯一）或 REQ-主题-YYYYMMDD（历史/例外） | 单一需求文档为 我的需求.md，用户在其中按条追加；每条需求须带**唯一编号**（见 2.1.1）、**状态**（待实现、进行中、已完成、废弃、未来实现）、可选**关联 PRD**、可选**关联 DD**；主题与日期在文档内每条标题与日期中体现。需求审核、疑问与结论在 02-PRD 内完成，不在 01 中填写。 | 我的需求.md、REQ-GUI-xxx-20260210.md（历史保留）                                                          |
| 02-产品需求文档PRD | **PRD-模块**简短描述.md                                        | 前缀 PRD-，与需求主题一致；主题中含 -GUI-/-APP-/-TOOL- 范围标识。**模块化 PRD 允许增量更新，见 10.1 节。**                                                                                                                                                                                    | PRD-GUI-页面风格改为Fluent-20260210.md、PRD-GUI-APP-培养箱设置按钮-20260209.md                            |
| 03-架构设计        | **ARCH-模块**简短描述.md                                       | 前缀 ARCH-；主题中含 -GUI-/-APP-/-TOOL- 范围标识                                                                                                                                                                                                                                                    | ARCH-APP-系统分层-20260201.md                                                                             |
| 04-设计文档        | **DD-模块**简短描述.md                                         | 前缀 DD-（Design Doc），与 PRD 主题一致；主题中含 -GUI-/-APP-/-TOOL- 范围标识                                                                                                                                                                                                                       | DD-GUI-页面风格改为Fluent-20260210.md                                                                     |
| 05-经验手册        | **fixbug-** 或 **经验-** + 主题**-YYYYMMDD**.md          | bug 相关用 fixbug-，非 bug 用 经验-；主题中含 -GUI-/-APP-/-TOOL- 范围标识                                                                                                                                                                                                                           | fixbug-APP-培养箱数据库first_cap_delay_time改进-20260202.md、经验-Git中文提交信息乱码解决方案-20260126.md |
| 06-开发记录        | **开发记录-YYYYMMDD**.md                                       | 按日期，单日单文件                                                                                                                                                                                                                                                                                  | 开发记录-20260210.md                                                                                      |
| 07-运维手册        | **OPS-**主题简短描述**-YYYYMMDD**.md                     | 前缀 OPS-；主题中含 -GUI-/-APP-/-TOOL- 范围标识                                                                                                                                                                                                                                                     | OPS-APP-部署说明-20260210.md                                                                              |
| 08-提示词          | **提示词-YYYYMMDD**.md                                         | 按日期                                                                                                                                                                                                                                                                                              | 提示词-20260210.md                                                                                        |
| 09-重构记录        | **重构-**主题简短描述**-YYYYMMDD**.md                    | 前缀 重构-；主题中含 -GUI-/-APP-/-TOOL- 范围标识                                                                                                                                                                                                                                                    | 重构-GUI-设置页数据刷新唯一路径-20260206.md                                                               |
| 10-分析报告        | **REPORT-**主题简短描述**-YYYYMMDD**.md                  | 前缀 REPORT-；主题中含 -GUI-/-APP-/-TOOL- 范围标识                                                                                                                                                                                                                                                  | REPORT-APP-模拟器get_cap_time持续刷新根因分析-20260210.md                                                 |
| 11-接口管理        | **API-**主题简短描述**-YYYYMMDD**.md                     | 前缀 API-；主题中含 -GUI-/-APP-/-TOOL- 范围标识（按接口所属）                                                                                                                                                                                                                                       | API-APP-request接口说明-20260201.md                                                                       |
...
- **范围标识（-GUI- / -APP-）**：主题简短描述中须按文档涉及范围加入标识。**仅涉及 GUI**（gui/、页面、控制器、前端交互）的文档，主题中须含 **-GUI-**；**仅涉及 APP**（app/、后端、设备、业务逻辑、接口层）的文档，主题中须含 **-APP-**；**同时涉及 GUI 与 APP** 的文档，**-GUI-** 与 **-APP-** 均须包含（顺序可任意，如 `REQ-GUI-APP-xxx` 或 `REQ-APP-GUI-xxx`）。06 开发记录、08 提示词无范围标识要求。
- **日期 YYYYMMDD**：文档创建或重大修订日期，必须为 8 位数字。
- **扩展名**：统一 .md。

#### 与 YAML doc_id 的对应关系

- 文档内 YAML 头的 `doc_id` 建议与文件名对应，如：文件 `DD-GUI-页面风格改为Fluent-20260210.md` → `doc_id: DD-GUI-页面风格改为Fluent-20260210`（与现有约定一致即可）。
...
### 2.2 02-产品需求文档 (PRD) 编写规范【强制】

**所有 02-产品需求文档 (PRD) 必须严格遵循以下 Markdown 模板结构编写。**
...
## 2. 变更记录
- **YYYY-MM-DD**: [需求编号] 变更说明

---

## PRD-需求1：需求1标题 (待实现|进行中|已完成|取消)
